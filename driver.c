...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT driverObject, IN PUNICODE_STRING registryPath) {
    ...
    NTSTATUS Status; //van 2019 

    CHAR textCreate_[64] = "INFO_VAN000 \r\n";
    size_t lengthCreate_ = 64;
    RtlStringCchLengthA(textCreate_, 64, &lengthCreate_);
    Status = VanLogCreat(textCreate_, (ULONG)lengthCreate_);
    if (!NT_SUCCESS(Status))
    {
    ...
    CHAR text_[64] = "INFO_VAN001 \r\n";
    size_t length_ = 64;
    RtlStringCchLengthA(text_, 64, &length_);
    Status = VanLogWrite(text_, (ULONG)length_);
    if (!NT_SUCCESS(Status))
    {
...
NTSTATUS EvtDeviceAdd(IN WDFDRIVER Driver, IN PWDFDEVICE_INIT DeviceInit) {
    ...
    TraceVerbose(DBG_INIT, "(DriverVan=0x%p)", Driver);
...
NTSTATUS VanLogCreat(CHAR text[], ULONG length)
// Create a WDF IO queue for a DMA engine
{
  NTSTATUS status = STATUS_SUCCESS;

  //PCUCHAR buffer[] = { 0x00, 0x01, 0x02, 0x03 };    //  for example
  //ULONG buffer[] = { 0x00, 0x01, 0x02, 0x03 };    //  for example
  //ULONG bufferSize = sizeof(buffer);
  //ULONG buffer = 0x13;    //  for example
  //ULONG bufferSize = sizeof(ULONG);

  //UNICODE_STRING      filePath = "\??\C:\Temp\logs.txt";   //  Must be with DOS prefix: \??\C:\Temp\logs.txt
  //PCWSTR filePath_ = L"\\??\\C:\\Temp\\logs.txt";
  //UNICODE_STRING      filePath = RTL_CONSTANT_STRING(filePath_);   //  Must be with DOS prefix: \??\C:\Temp\logs.txt

  //static WCHAR filePath_[] = L"\\??\\C:\\Temp\\logs.txt";
  static WCHAR filePath_[] = L"\\??\\C:\\Temp\\logs.txt";
  static UNICODE_STRING filePath = RTL_CONSTANT_STRING(filePath_);
  static HANDLE              hFile;
  ...
  //static WCHAR text_[] = L"Hello world";
  //static UNICODE_STRING text = RTL_CONSTANT_STRING(text_);

  //static CHAR text_[64] = "Hello world";
  //size_t length_ = 64;
  //RtlStringCchLengthA(text_, 64, &length_);
  PVOID buffer = text;
  ULONG bufferSize = length; // sizeof(text_);
...
//NTSTATUS VanLogWrite(void)
NTSTATUS VanLogWrite(CHAR text[], ULONG length)
// Create a WDF IO queue for a DMA engine
{
  NTSTATUS status = STATUS_SUCCESS;
  ...
  NTSTATUS Status = ZwOpenFile(&hFile, FILE_APPEND_DATA, &ObjectAttributes, &IoStatusBlock,
    0, FILE_SYNCHRONOUS_IO_NONALERT);
  if (!NT_SUCCESS(Status))
  {
    DbgPrint("[DRV_NAME]: Opening file error");
    return Status;
  }
